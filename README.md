...
- **V1.0**：核心闭环——今日概览、项目、任务、日历、时间节点、灵感速记、工具箱、进展日志、数据目录配置与备份；V1.2 增加词汇库管理（标签库 + 自定义节点类型）与全屏流式布局。
- **V2.0**：产出增强——任务看板视图（拖拽换状态）、论文投稿跟踪（状态流转/章节任务联动/日期自动生成节点/录用入台账）、周报月报与工作总结自动生成（模板可自定义）、成果台账（时间线/复制纯文本）、汇报导出（Markdown + Word docx，命名 `周报_2026_第34周_YYYYMMDD.docx`）。
